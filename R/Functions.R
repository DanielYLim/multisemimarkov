...
#'
#' @param alpha1 is a scale parameter.
#' @param beta1  is a shape parameter.
#' @param t is a time to event where t>0.
#'
...
#'
#' @param k is a shape parameter.
#' @param lambda is a scale parameter.
#' @param t is time to event where t>0.
#'
#' @return haaard function of t.
#' @export
#'
#' @examples
#' hazardWei(1,2,1.3)
...
#' @param lambda14 Weibull scale parameter; 1->4 transition.
#' @param k14 Weibull shape parameter; 1->4 transition.
...
#'
#' @examples
#' Data_temp = data_generation(n=200, alpha12=2.5, beta12=2, alpha13=5, beta13=2.5, lambda23=1, k23=2, lambda04=10, k04=4, lambda14=4, k14=2, tau=0.3, p=0.7, Pg_mb=0.3, Pg_mc=0.2, C_max=10, tau_0=2)
#' head(Data_temp)
#'
#'
data_generation = function(n=200, alpha12=2.5, beta12=2, alpha13=5, beta13=2.5, lambda23=1, k23=2, lambda04=10, k04=4, lambda14=4, k14=2, tau=0.3, p=0.7, Pg_mb=0.3, Pg_mc=0.2, C_max=10, tau_0=2){

  gett<-function(t){
    Sol<-log(1-u)+log(1+(t/alpha12)^(beta12))+log(1+(t/alpha13)^(beta13))+(t/lambda14)^(k14)
    return(Sol)
  }
...
for (i in 1:n){
	if (W[i]==1){
		...
		pi1<-hazardLL(alpha12,beta12,t)/(hazardLL(alpha12,beta12,t)+hazardLL(alpha13,beta13,t)+hazardWei(k14, lambda14, t))
		pi2<-hazardLL(alpha13,beta13,t)/(hazardLL(alpha12,beta12,t)+hazardLL(alpha13,beta13,t)+hazardWei(k14, lambda14, t))
		pi3 = 1-(pi1+pi2)
		prob = c(pi1, pi2, pi3)
