...
#' Estimation unmasked data with two stage method
#'
#' @param n number of observation
#' @param alpha12 log-logstic scale parameter; 1->2 transition.
#' @param beta12 log-logstic shape parameter; 1->2 transition.
...
#' alpha04 = 7
#' beta04 = 4
#' Result <- Estimation_unmasked_two_stage(n, alpha12, beta12, alpha13, beta13, alpha23, beta23, tau, p, C_max, alpha04, beta04)
#' print(Result$estimate)
Estimation_unmasked_two_stage<-function(n=200, alpha12 = 2, beta12 = 4, alpha13 = 3.5, beta13 = 3, alpha23 = 2.5, beta23 = 1.5, tau = 0.3, p = 0.7, C_max = 10, alpha04 = 7, beta04 = 4){
  ...
  pi1<- cubature::adaptIntegrate(f10j, lowerLimit=0, upperLimit=20, alphajk=alpha12hat, betajk=beta12hat, alphajl=alpha13hat, betajl=beta13hat)$integral

  F101.star.hat.new = F101 / pi1
  ...
  estimates_2 <- nlm(log_lik_second_stage,initial_parms)$estimate

  Data_out <- list()
  Data_out$estimates <- c(estimates[1:4], estimates_2, estimates[5])
  Data_out$n <- n
  Data_out$Cenrate_12 <- Cenrate_12
  Data_out$Cenrate_13 <- Cenrate_13
  Data_out$Cenrate_23 <- Cenrate_23
...
#' Simulation unmasked data using two stage method
#'
#' @param B number of simulation runs
#' @param n number of observations.
#' @param alpha12 log-logstic scale parameter; 1->2 transition.
...
#' alpha04 = 7
#' beta04 = 4
#' Sim_result <- Simulation_unmasked_two_stage(B, n, alpha12, beta12, alpha13, beta13, alpha23, beta23, tau, p, C_max, alpha04, beta04)
#' total_num <- length(Sim_result)
#'estimations <- c()
#'for (i in 1:total_num){
#'  estimation_temp <- Sim_result[[i]]$estimate
...
#'}
#'print(estimations)
Simulation_unmasked_two_stage<-function(B=5, n=200, alpha12 = 2, beta12 = 4, alpha13 = 3.5, beta13 = 3, alpha23 = 2.5, beta23 = 1.5, tau = 0.3, p = 0.7, C_max = 10, alpha04 = 7, beta04 = 4){

  Data_out <-list()
  for (i in 1:B){
    Data_out_temp <- Estimation_unmasked_two_stage(n, alpha12, beta12, alpha13, beta13, alpha23, beta23, tau, p, C_max, alpha04, beta04)
    Data_out[[i]] <- Data_out_temp
  }
